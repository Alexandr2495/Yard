...
- **Когда использовать:** 
  - После изменения постов в каналах
...
### 🔄 **Для мониторинга:**
1. **Регулярно запускайте `/rescan`** после изменения постов
2. **Проверяйте список мониторимых постов** командой `list_monitored_posts`
3. **Удаляйте устаревшие посты** из мониторинга
...
2. **Регулярный перескан:** `/rescan` после любых изменений
3. **Проверка мониторинга:** `list_monitored_posts` для контроля
4. **Диагностика:** `/diag` при проблемах
